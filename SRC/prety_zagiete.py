...
def oblicz():
    try:
        ...
        UMBUGELUNG.config(text=f"UMBUGELUNG: { math.ceil((  (float(ROZ_PR_PIO_SB1.get())) + (float(ROZ_PR_PIO_SB1.get())) + (float(PR_1LD.get()) * 1.15) + (float(SR_UBL.get()) * 1.15) + (float(SR_UBL.get()) * 1.15)  ) /10)}cm")
